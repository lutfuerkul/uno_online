...
// Reverse kilidi varken oynanabilir: aynı renk, başka reverse ya da joker.
...
async function playCard(cardId, chosenColor, targetId) {
  ...
  await runTransaction(db, async (tx) => {
    ...
    // Reverse sonrası özel kısıt: aynı renk, başka reverse ya da joker.
...
function renderBoard() {
  ...
  // Reverse kilidi varken aynı renk, reverse ya da joker oynanabilir.
    ...
    <div class="screen">
      ...
      <div class="turn ${isMyTurn ? "mine" : "theirs"}">
        ...
        ${isMyTurn && reverseColor != null
          ? `<div class="hint">↩️ Reverse! Sadece <b>${COLOR_TR[reverseColor] || reverseColor}</b>, başka bir Reverse ya da Joker oyna — yoksa çek/pas.</div>`
          : ""}
      </div>
...
async function tryPlay(cardId) {
  ...
  if (!ok) {
    return toast(reverseColor != null
      ? `Reverse sonrası sadece ${COLOR_TR[reverseColor] || ""}, Reverse ya da Joker oynayabilirsin.`
      : "Bu kart oynanamaz.");
  }
